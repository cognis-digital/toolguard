...
**toolguard** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `toolguard` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
toolguard ... --format json | cognis-connect emit --to brief   # analyst brief via your /v1 fleet
toolguard ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `toolguard` outputs:

```python
from cognis_connect import normalize, edgemesh
findings = [normalize(rec, source="toolguard") for rec in records]   # records = your JSON output
